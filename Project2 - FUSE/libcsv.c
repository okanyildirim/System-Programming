...
size_t
csv_parse(struct csv_parser *p, const void *s, size_t len, void (*cb1)(void *, size_t, void *),
    ...
    while (pos < len) {
        ...
        switch (pstate) {
            ...
            case FIELD_BEGUN:
                ...
                } else if (!quoted && (is_space ? is_space(c) : c == CSV_SPACE ||
                                                                c == CSV_TAB)) { /* Tab or space for non-quoted field */
                    SUBMIT_CHAR(p, c);
                    spaces++;
                } else {  /* Anything else */
                    SUBMIT_CHAR(p, c);
